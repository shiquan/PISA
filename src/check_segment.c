...
static int usage()
{
    fprintf(stderr, "* Pick pre-designed segments from reads.\n");
...
static int check_segment_core(struct segment *G, char *s)
{
    khint_t k;
...
static char **check_pattern(char *s, int start, struct ref_pat *r, struct hit *h, int *partly_found)
{
    ...
    for (i = 0; i < r->n; ++i) {
        ...
        if (seg->n > 0) {
            int e = check_segment_core(seg, str.s);
            if (e < 0) { // unmatched
                int j;
                for (j = 0; j < r->n; ++j)
                    if (fetch[j]) free(fetch[j]);
...
int check_segment(int argc, char **argv)
{
    double t_real;
    t_real = realtime();
