...
task parseFastq {
  ...
  String root
  command {
    ${root}/SingleCellTools parse -t 15 -f -q 20 -config ${config} -cbdis ${outdir}/temp/barcode_counts_raw.txt -report ${outdir}/temp/sequencing_report.json ${fastq1}  > ${outdir}/temp/reads.fq
  }
  output {
...
task fastq2bam {
  ...
  String root
  command {
    ${bwapath} mem -t 24 ${refdir} ${fastq} | ${root}/SingleCellTools sam2bam -o ${outdir}/temp/aln.bam -report ${outdir}/temp/alignment_report.txt -maln ${outdir}/temp/mito.bam /dev/stdin
  }
  output {
